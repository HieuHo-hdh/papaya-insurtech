...
## Detail
Create `source/fe/lib/theme.ts` — maps `TenantConfig.branding` to Ant Design 6 theme tokens:
```typescript
...
Create `source/fe/components/providers/AntdProvider.tsx`:
```tsx
'use client'
...
Add `AntdProvider` to `source/fe/app/layout.tsx`:
```tsx
