...
## Description
Define all shared TypeScript types and Zod 4 schemas in `source/shared/`, then copy them to `be/src/shared/` and `fe/shared/`.

## Detail
Create the following files in `source/shared/`:
...
- `cp source/shared/types.ts source/be/src/shared/types.ts`
- `cp source/shared/schemas.ts source/be/src/shared/schemas.ts`
(FE copy is handled by T013)

The copy step can be scripted as `source/shared/sync.sh`:
```bash
#!/bin/bash
...
## Expectation
`source/shared/types.ts` and `schemas.ts` compile with zero TypeScript errors. `be/src/shared/` and `fe/shared/` contain identical copies.

## Acceptance Criteria
- [ ] `source/shared/types.ts` exports all types from architecture.md TenantConfig shape
- [ ] `source/shared/schemas.ts` exports `TenantConfigSchema` with all validation rules
