...
## Description
Add a `docker-compose.yml` at `challenges/` that runs PostgreSQL and the BE service together for local development.

## Detail
Create/update `challenges/docker-compose.yml` with two services:
...
- `be`: builds from `challenges/be/Dockerfile`, depends on `db`, port `3001:3001`, `env_file: ./be/.env`, mounts `./be:/app` for hot reload in dev

Add `challenges/be/Dockerfile` (multi-stage is optional for now — single-stage dev image is fine):
```dockerfile
FROM node:20-alpine
WORKDIR /app
COPY package*.json ./
...
- [ ] `challenges/be/Dockerfile` exists and builds successfully
- [ ] `challenges/be/.env.example` exists with all required vars documented
