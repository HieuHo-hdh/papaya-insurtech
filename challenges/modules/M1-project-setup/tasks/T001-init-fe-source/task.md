...
## Description
Bootstrap the Next.js 16 App Router project with TypeScript, Ant Design 6, and Tailwind v4 in `challenges/fe/`.

## Detail
Create a new Next.js 16 project inside `challenges/fe/` using the App Router. Configure:
...
- `challenges/fe/package.json`
- `challenges/fe/tsconfig.json`
- `challenges/fe/next.config.ts`
- `challenges/fe/postcss.config.mjs`
- `challenges/fe/app/globals.css`
- `challenges/fe/app/layout.tsx` (stub — adds AntdProvider once T011 done)
- All placeholder route files per coding-standards.md

## Expectation
`cd challenges/fe && npm install && npm run dev` starts on port 3000 with no errors. TypeScript check (`npm run build`) passes on stub pages.
