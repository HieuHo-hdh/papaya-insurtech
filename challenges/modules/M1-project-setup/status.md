...
### T003 · Docker Compose
- `challenges/docker-compose.yml` already present with `postgres` + `be` services
- `be/Dockerfile` already present

### T004 · Shared types + schemas
- `challenges/shared/types.ts` and `schemas.ts` already fully defined
- Copied to `be/src/shared/` and `fe/shared/`
- Created `challenges/shared/sync.sh` to re-sync on future changes
...
- `fe/shared/types.ts` and `fe/shared/schemas.ts` already present and in sync with `challenges/shared/`
